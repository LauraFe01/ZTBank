...
user=""  # variabile globale per username
...
doc_type_select() {
  ...
  echo "3) Documenti Operativi"
  read -p "Scelta: " dt_choice
  case $dt_choice in
    1) DOCUMENT_TYPE="Dati Transazionali" ;;
    2) DOCUMENT_TYPE="Dati Personali" ;;
    3) DOCUMENT_TYPE="Documenti Operativi" ;;
    *) echo "Scelta non valida, default Dati Transazionali"; DOCUMENT_TYPE="Dati Transazionali" ;;
  esac
}

# Funzione per selezione sensibilita
group_select() {
  if [[ "$user" == "giovanni.rossi" ]]; then
    echo "Seleziona livello di sensibilità:"
    echo "1) sensibile"
    ...
    case $s_choice in
      ...
      *) echo "Scelta non valida, default non_sensibile"; SENSITIVITY="non_sensibile" ;;
    esac
  else
    echo "Utente '$user' non autorizzato a selezionare sensibilità. Impostato default non_sensibile."
    SENSITIVITY="non_sensibile"
  fi
}
...
login() {
  ...
  curl -s -X POST http://$PEP_IP:$PEP_PORT/login \
       ...
       | $JQ_CMD
  echo "Login effettuato come '$user' e cookie salvati in $COOKIE_FILE"
}
